...
class UserRepo:
    def __init__(self, pg_manager):
        ...
        self.users_table = Table(
            ...
            Column("role_id", Integer, ForeignKey("react_schema.roles.id")),
    ...
    def find_user_by_credentials(self, email, password_hash):        
        with self.pg_manager.session_scope() as sess:
            ...
            # Construct a statement to select the user's ID, username, and role name
            stmt = select(self.users_table.c.id, self.users_table.c.username, self.roles_table.c.name.label('role_name')).select_from(join).where(self.users_table.c.email == email,self.users_table.c.password == password_hash)
