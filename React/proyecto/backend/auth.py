...
def login_user():    
    ...
    if not data or not all(k in data for k in ['email', 'password']):
        return Response(status=400, response="Missing email or password.")  
   
    hashed_password = hashlib.sha256(data['password'].encode()).hexdigest()
    user = user_repo.find_user_by_credentials(data['email'], hashed_password)
    if user:        
        token = jwt_manager.encode({'id': user.id, 'role': user.role_name})
        return jsonify(token=token, user={"name": user.username, "role": user.role_name}), 200
    else:
