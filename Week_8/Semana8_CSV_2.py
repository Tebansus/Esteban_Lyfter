...
def save_to_csv(file_name, games):
    try:
        with open(file_name, mode='w', newline='', encoding='utf-8') as file:
            # Key change is the "Delimiter" p[arameter that can be adjusted to change the separating value
            writer = csv.DictWriter(file, fieldnames=["name", "genre", "developer", "classification"], delimiter='\t')
            writer.writeheader()
...
def main():
    try:
        ...
        if games:
            save_to_csv("Saved_Games_TAB_List.txt", games)
